...
async function storeEmbeddings(){
    await Promise.all(
        dataToEmbed.map(async (item, index) => {
            //first request to embedding info
            const embeddingResult = await openai.embeddings.create({
                model: 'text-embedding-3-small',
                input: item.info
            })
            const embedding = embeddingResult.data[0].embedding;
            //second request to save info embedded into vector database
...
async function queryEmbeddings(question: string) {
    //fist request to embedding the question
    ...
    //second request to query the vector database with embedding question
...
async function main() {
    ...
    const relevantInfo = result.matches[0].metadata?.info;
    if(relevantInfo) {
        await askOpenAI(question, relevantInfo)
    }
}
